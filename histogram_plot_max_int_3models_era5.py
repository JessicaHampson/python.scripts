...
for i, md in enumerate(mn):  # model: LOW, MED, HI
    ...
    for j, ens in enumerate(tqdm(ensset, desc=f'{md} processing')):
        ...
        if ti_key_name not in f.files or num_key_name not in f.files:
            print(f'Keys {ti_key_name} or {num_key_name} not found in {file_path}')
            continue
...
bins = np.linspace(0,30,16)  # set bin interval
xticks = np.arange(0,20,5)  # set x axis ticks
...
plt.legend(loc='upper right', title='ERA5 vs. Ensemble Mean',title_fontsize=16,fontsize=15)
plt.yticks(yticks)
ax = plt.gca()
ax.set_ylim(0,120)  # set y limit
ax.set_xlim(0,20)  # set x limit
ax.tick_params(axis='both',labelsize=16)
ax.set_ylabel("Frequency", fontsize = 20)
ax.set_xlabel("Lifetime (days)", fontsize= 20)
ax.set_title("Bomb Cyclone Lifetime", fontsize= 20)
plt.tight_layout()
plt.show()
